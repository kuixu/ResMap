...
def ResMap_algorithm(**kwargs):
	...
	# Grab the volume size (assumed to be a cube, if not filled with min-value)
	n = data.shape[0]
	if n**3 != data.shape[0]*data.shape[1]*data.shape[2]:
		
		x0,y0,z0= data.shape
		import copy
		data_bak = copy.deepcopy(data)
		n = max(data.shape)
		print("raw shape:", data.shape, "new shape: (", n,n,n,")")
		data = np.zeros((n,n,n)) + data_bak.min()
		data[:x0,:y0,:z0] = data_bak
	...
	while moreToProcess:
		...
		if kmax == 0:
			...
			maskSumConst = np.sum(1.0/np.array(range(1,int(maskSum))))
